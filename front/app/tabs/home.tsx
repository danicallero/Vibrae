...
export default function HomePage(): JSX.Element {
  ...
  useEffect(() => {
    ...
    function connectWs() {
      ws = new WebSocket(
        API_URL.startsWith("http") ? API_URL.replace(/^http/, "ws") + "/control/ws" : "ws://localhost:8000/control/ws"
      );
      wsRef.current = ws;
      ws.onopen = () => {};
      ws.onclose = () => {
  ...
  return (
    <SafeAreaView style={styles.container}>
      <View style={styles.content}>
        ...
        <ScrollView style={{ flex: 1 }} contentContainerStyle={{ paddingBottom: 64 }} showsVerticalScrollIndicator={false}>
          {loading ? (
            ...
            <View style={[styles.balanceCard, { marginBottom: 24 }]}>
              ...
              <TouchableOpacity style={[styles.deleteButton, { marginTop: 10 }]} onPress={handleStop}>
                <Ionicons name="stop-circle" size={18} color={COLORS.text} />
            ...
            <View style={[styles.emptyState, { marginBottom: 24 }]}>
              ...
              <Text style={styles.emptyStateText}>Actualmente no hay ninguna canción en reproducción.</Text>
              {shouldBePlaying && (
                <TouchableOpacity
                ...
                >
                  ...
                  <Text style={styles.addButtonText}>Reanudar {scheduleName ? `“${scheduleName}”` : "rutina"}</Text>
                </TouchableOpacity>
              )}
            </View>
          )}
          ...
          <View style={[styles.balanceCard, { marginBottom: 32, padding: 17 }]}>
            <Text style={{ fontSize: 16, fontWeight: "600", color: COLORS.text, marginBottom: 12 }}>Volumen: {Math.round(volume)}%</Text>
            ...
            <Slider
              ...
              theme={{ maximumTrackTintColor: COLORS.border, minimumTrackTintColor: COLORS.primary }}
              renderThumb={(thumbWidth = 50, thumbTouchSize = 60, disableTrackPress = true) => (
                <RNView style={{ backgroundColor: COLORS.primary, borderColor: COLORS.primary, borderWidth: 2, width: 16, height: 16, borderRadius: 10 }} />
              )}
              renderBubble={() => null}
          ...
          <View style={{ gap: 16 }}>
            <TouchableOpacity style={[styles.addButton, { paddingVertical: 14, borderRadius: 28 }]} onPress={() => router.push("/tabs/routines")}>
              <Ionicons name="calendar-outline" size={20} color={COLORS.textLight} />
              <Text style={[styles.addButtonText, { fontSize: 16 }]}>Rutinas</Text>
            </TouchableOpacity>

            <TouchableOpacity style={[styles.addButton, { paddingVertical: 14, borderRadius: 28 }]} onPress={() => router.push("/tabs/scenes")}>
        ...
        <TouchableOpacity onPress={handleLogout} style={[styles.logoutButton, { position: "absolute", top: 20, right: 20 }]}>
          <Ionicons name="log-out-outline" size={20} color={COLORS.text} />
        </TouchableOpacity>
      </View>

      <View style={styles.footer}>
        <Text style={styles.footerText}>Made with ❤️ by Dani Callero</Text>
      </View>
    </SafeAreaView>
  );
}